 ...
 * @author Aluno
 */
public class ListaCompraControl {
    ...
    public static void atualizarStatus(int row,boolean status){
        ArrayList<ItemCompra> iC = ItemCompra.getCompras();
        iC.get(row).setStatus(status);
        iC.get(row).Persistir(row);
    }
